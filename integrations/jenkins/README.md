...
You are now good to go perform an inital run of the pipeline by clicking on *Build Now*. This will not yet run ORT, but
just configure the pipeline parameters and its stages, and this run is expected to "fail".
...
Now the pipeline is properly configured / initialized and you can click again on *Build with Parameters* to run it on
VCS repositories Jenkins as (read-)access to. Of course, you can also programmatically trigger the ORT pipeline from
your application build pipelines.
